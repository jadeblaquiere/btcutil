...
func (cs *CoinSet) TotalValue() (value cttutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue cttutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue cttutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount cttutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue cttutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue cttutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue cttutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue cttutil.Amount, coins []Coin) (Coins, error) {
...
// cttutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() cttutil.Amount {
	return cttutil.Amount(c.txOut().Value)
}
