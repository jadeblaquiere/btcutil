...
import (
	...
	"github.com/jadeblaquiere/cttutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := cttutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(cttutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := cttutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := cttutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := cttutil.NewBlockFromBytes(blockBytes)
	if err != nil {
